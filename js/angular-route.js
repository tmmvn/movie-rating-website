...
( function(window, angular)
    ...
    function routeToRegExp(path, opts)
      {
        ...
        var pattern = path
          ...
          .replace(/(\/)?:(\w+)(\*\?|[?*])?/g, function(_, slash, key, option)
          {
            ...
            slash = slash || ""
            return ( ( optional ? "(?:" + slash : slash + "(?:" ) + ( star ? "(.+?)" : "([^/]+)" ) +
                     ( optional ? "?)?" : ")" ) )
          })
          .replace(/([/$*])/g, "\\$1")
   ...
   *      controller} if passed as a string.
        ...
        this.when = function(path, route)
          {
            ...
            routes[path] = angular.extend(routeCopy, { originalPath: path }, path && routeToRegExp(path, routeCopy))
        ...
        this.$get = ["$rootScope", "$location", "$routeParams", "$q", "$injector", "$templateRequest", "$sce",
          "$browser", function($rootScope, $location, $routeParams, $q, $injector, $templateRequest, $sce, $browser)
            {
              /**
               * @ngdoc service
               * @name $route
                  ...
                  if (preparedRouteIsUpdateOnly)
                  ...
                  else if (nextRoute || lastRoute)
                    {
                      ...
                      nextRoutePromise.then(getRedirectionData)
                        ...
                        {
                          return keepProcessingRoute && nextRoutePromise.then(resolveLocals)
                  ...
                  if (data.route !== $route.current)
                  ...
                  else if (data.hasRedirection)
                    {
                      ...
                      if (newUrl)
                        ...
                        {
                          newUrl = $location.path(data.path)
                            .search(data.search)
                            .replace()
                            .url()
                        }
